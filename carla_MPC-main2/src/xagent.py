...
class Xagent(BasicAgent):
    ...
    def run_step(self):
        ...
        # 调试：检查预测轨迹是否有 NaN
        print(f"[DEBUG] state[2] has NaN: {np.isnan(state[2]).any()}, shape: {state[2].shape}")
        print(f"[DEBUG] xy coords: {state[2][:, :2]}")
        sys.stdout.flush()

        ca_u.draw_planned_trj(self._world, ref_traj[:2, :].T, height+0.5)
        ca_u.draw_planned_trj(self._world, state[2][:, :2], height+0.5, color=(0, 255, 255))
