...
def _debug_log_path():
    ...
    return os.path.join(log_dir, "run_debug.log")
...
class Vehicle:
    ...
    def solver_add_soft_obs(self, obs=None, ratio=500, expn=1):
        ...
        # ========== 经典人工势场法参数 ==========
        R_max = 10.0              # influence distance
        safe_dist = 2.30          # keep protection, but relax the final side-by-side squeeze a little
        k_u = 780.0               # slightly soften far/mid-range repulsion to reduce parking equilibria
        k_close = 1800.0          # reduce near-field stiffness once a valid side pass is already formed
        front_cost_dist = 17.0    # intervene a bit earlier before the first obstacle
        back_release_dist = 1.2   # release sooner once the obstacle has just moved behind the ego
        side_clearance = 2.75     # ask for a bit more lateral room to reduce scraping
        k_side = 300.0            # restore some lateral commitment
        v_min_near = 2.0          # still encourage progress, but less aggressively than before
        k_vnear = 72.0            # keep anti-crawl support without forcing a risky push
        k_brake_near = 72.0       # allow braking a bit more than the last aggressive version
        lateral_consider = 3.5    # ignore obstacles too far laterally (e.g., sidewalk)
        eps = 1e-3
        front_gate_den = max(front_cost_dist - R_max, 1e-3)
        
        for i in range(self.horizon):  # each prediction step
            state_i = self.X[:, i]
            ...
            for j in range(n_obs):
                ...
                preferred_side = obs[j, 3] if obs.shape[1] > 3 else 0
                ...
                release_gate = ca.if_else(
                    obstacle_ahead < -back_release_dist,
                    0.0,
                    rear_gate
                )
                ...
                # Side-pass bias. If a preferred side is provided, keep that side
                # to avoid left/right branch flips around centered obstacles.
                ...
                # Once we have committed to a passing side and already built
                # enough lateral room, reduce the close-range wall slightly so
                # the optimizer prefers finishing the pass instead of stopping
                # with the obstacle still a few decimeters ahead.
                ...
                # Penalize crawling/stopping near obstacle to reduce local-minimum parking.
                ...
                self.obj += gate_front * lateral_gate * post_pass_cost_scale * (
